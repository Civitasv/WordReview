...
// digits component, taking in the number of digits to show and mapping the
// digits around the clock
// for instance 4 digits --> 3, 6, 9, 12, positioned at 3, 6, 9, 12 o'click
// respectively
var Digits = function Digits(_ref) {var howMany = _ref.howMany,distance = _ref.distance;
  ...
  // describe the text elements rotated according to the digit number (12 at
	// the very top, 3 at the very right and so forth repeating the structure of
	// an analog clock)
...
// hours component, taking the current hours and drawing a hand rotated as per
// the number of hours
// 12 hours clock
...
// minutes component, taking the current minutes and drawing a hand rotated as
// per the number of minutes
...
// seconds component, taking the current seconds, drawing 60 ticks for each
// second in the dial and highlighting the current second
...
var SVGatch = function (_Component) {_inherits(SVGatch, _Component);
  function SVGatch(props) {_classCallCheck(this, SVGatch);
    ...
    // additionally define root variables for the size of the SVG and the margin
	// allowing to nest SVG element inside, without cropping
    // following d3's margin convention
    // ! turn addded to keep track of the number of times the seconds marker
	// goes around the clock, and in so doing avoiding the 'snap' occurring
	// between 359 degrees and 0
    // this way, instead of going back to rotate(0), the hands and marker go to
	// rotate(360), then rotate(361) and so forth and so on
  ...
  // when the components are mounted set up an interval to update the date
	// object every second
  _createClass(SVGatch, [{ key: "componentDidMount", value: function componentDidMount() {var _this2 = this;
      this.interval = setInterval(function () {
        var date = new Date();
        // check if the number of seconds is 0, and in this instance increment
		// the turn variable of the matching element
...

    {
      ...
      /*
		 * define transform values to translate the SVG elements (per d3's
		 * margin convention) and to center the elements
		 */
      ...
      /*
		 ...
		 * circle, for the outer most ring Digits, for text elements distributed
		 * around the ring Hours, for the hour's hand Minutes, for the minute's
		 * hand Seconds, for the ticks representing the seconds and a marker
		 * highlighting the current second
		 */
      return (
